...
// const StatusDropdown = ({ stage, value, onChange }: StatusDropdownProps) => {
//   const availableStatuses = STAGE_STATUS_MAP[stage] || [];
...
const StatusDropdown = ({ applicant, onUpdate }: StatusDropdownProps) => {
  ...
  const availableStatuses = useMemo(() => {
    return STAGE_STATUS_MAP[currentStage as keyof typeof STAGE_STATUS_MAP] || STAGE_STATUS_MAP.screening;
  }, [currentStage]);
  ...
  const currentStatus = applicant.status || STAGE_DEFAULT_STATUS[currentStage as keyof typeof STAGE_DEFAULT_STATUS] || "";
