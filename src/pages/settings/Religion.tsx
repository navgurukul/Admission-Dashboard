...
export default function Religion() {
  ...
  const fetchReligions = async () => {
    try {
      ...
      toast({
        title: "Error",
        description:
          error instanceof Error ? error.message : "Failed to fetch religions",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleAddReligion = async () => {
    ...
    if (
      ...
      religions.some(
        (religion) =>
          religion.religion_name.toLowerCase() ===
          newReligion.religion_name.toLowerCase(),
      )
    ) {
      toast({
        title: "Error",
    ...
    try {
      setCreating(true);
      const createdReligion = await createReligion({
        religion_name: newReligion.religion_name,
      });
      setReligions(
        Array.isArray(religions)
          ? [...religions, createdReligion]
          : [createdReligion],
      );
      ...
      toast({
        title: "Error",
        description:
          error instanceof Error ? error.message : "Failed to create religion",
        variant: "destructive",
      });
    } finally {
      setCreating(false);
  ...
  const handleUpdateReligion = async () => {
    ...
    if (
      ...
      religions.some(
        (religion) =>
          ...
          religion.religion_name.toLowerCase() ===
            editReligion.religion_name.toLowerCase(),
      )
    ) {
      toast({
        title: "Error",
    ...
    try {
      const updatedReligion = await updateReligion(
        editingReligion.id.toString(),
        { religion_name: editReligion.religion_name },
      );
      setReligions(
        Array.isArray(religions)
          ? religions.map((religion) =>
              religion.id === editingReligion.id ? updatedReligion : religion,
            )
          : [updatedReligion],
      );
      ...
      toast({
        title: "Error",
        description:
          error instanceof Error ? error.message : "Failed to update religion",
        variant: "destructive",
      });
    }
  };
  ...
  const handleDeleteReligion = async (id: number) => {
    ...
    try {
      ...
      setReligions(
        Array.isArray(religions)
          ? religions.filter((religion) => religion.id !== id)
          : [],
      );
      toast({
        title: "Success",
      ...
      toast({
        title: "Error",
        description:
          error instanceof Error ? error.message : "Failed to delete religion",
        variant: "destructive",
      });
    }
  };
      ...
      <div className="space-y-6" style={{ flex: 1 }}>
        <div>
          ...
          <p className="text-gray-600">
            Manage religious categories and classifications
          </p>
        </div>
            ...
            <div className="flex gap-4">
              <div className="flex-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNewReligion({ religion_name: e.target.value })
                  }
                  ...
                  onKeyDown={(e) => {
                    if (
                      e.key === "Enter" &&
                      newReligion.religion_name.trim() &&
                      !creating
                    ) {
                      handleAddReligion();
                    }
              ...
              <div className="flex items-end">
                <Button
                  ...
                  disabled={creating || !newReligion.religion_name.trim()}
                >
                  {creating ? (
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Manage current religious categories
                </CardDescription>
              </div>
              <Button
                variant="outline"
                      ...
                      <div className="flex items-center gap-4 flex-1">
                        <div className="flex-1">
                          <Input
                            ...
                            onChange={(e) =>
                              setEditReligion({ religion_name: e.target.value })
                            }
                            placeholder="Religion name"
                            onKeyDown={(e) => {
                              if (
                                e.key === "Enter" &&
                                editReligion.religion_name.trim()
                              ) {
