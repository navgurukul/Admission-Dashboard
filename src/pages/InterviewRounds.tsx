...
const InterviewRounds = () => {
  ...
  const learningRoundPass = filteredApplicants.filter((a) =>
    a.lr_status?.toLowerCase().includes("pass"),
  ).length;
  const culturalFitPass = filteredApplicants.filter((a) =>
    a.cfr_status?.toLowerCase().includes("pass"),
  ).length;
  const pendingInterviews = filteredApplicants.filter(
    (a) => a.lr_status && !a.cfr_status,
  ).length;
  const interviewFailures = filteredApplicants.filter(
    (a) =>
      a.lr_status?.toLowerCase().includes("fail") ||
      a.cfr_status?.toLowerCase().includes("fail"),
  ).length;
                        ...
                        <div className="flex flex-col items-center space-y-2">
                          ...
                          <span>
                            {searchQuery
                              ...
                              : "No applicants in interview rounds"}
                          </span>
                        </div>
                      </td>
                    </tr>
