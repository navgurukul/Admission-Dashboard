...
const Screening = () => {
  ...
  const programmingTrack = filteredApplicants.filter((a) =>
    a.qualifying_school?.toLowerCase().includes("programming"),
  ).length;
  const businessTrack = filteredApplicants.filter((a) =>
    a.qualifying_school?.toLowerCase().includes("business"),
  ).length;
  const averageScore =
    filteredApplicants.length > 0
      ? (
          filteredApplicants.reduce((sum, a) => sum + (a.final_marks || 0), 0) /
  ...
  const getStatusDisplay = (applicant: ApplicantData): string => {
    if (applicant.status === "pass") {
      if (applicant.qualifying_school?.toLowerCase().includes("programming")) {
        ...
        applicant.qualifying_school?.toLowerCase().includes("business")
      ) {
                        ...
                        <div className="flex flex-col items-center space-y-2">
                          ...
                          <span>
                            {searchQuery
                              ...
                              : "No applicants in screening stage"}
                          </span>
                        </div>
                      </td>
                    </tr>
                        ...
                        <td className="p-4">
                          <span
                            className={`text-lg font-bold ${
                              (applicant.final_marks || 0) >= 27
                                ? "text-status-active"
                                : (applicant.final_marks || 0) >= 18
