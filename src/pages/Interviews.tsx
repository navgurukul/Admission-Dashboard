          ...
          <div className="bg-card rounded-xl shadow-soft border border-border">
            <div className="p-6 border-b border-border">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold text-foreground">Interview Records</h2>
              ...
              <table className="w-full">
                <thead className="bg-muted/30">
                  <tr>
                    ...
                    <th className="text-left p-4 font-medium text-muted-foreground text-sm">Comments</th>
                      ...
                      <tr key={applicant.mobile_no} className="border-b border-border hover:bg-muted/20 transition-colors">
                        <td className="p-4">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">{applicant.unique_number || applicant.mobile_no}</p>
                          </div>
                        </td>
                        <td className="p-4">
                          <div className="flex items-center space-x-2">
                            <MessageSquare className="w-4 h-4 text-muted-foreground" />
                            <span className="text-sm text-foreground">{getInterviewType(applicant)}</span>
                          </div>
                        </td>
                        <td className="p-4">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-sm text-foreground">
                              {applicant.date_of_testing ? new Date(applicant.date_of_testing).toLocaleDateString() : 'Not scheduled'}
                            </span>
                          </div>
                        </td>
                        <td className="p-4">
                          <StatusBadge status={getInterviewStatus(applicant) as any} />
                        </td>
                        <td className="p-4">
                          <span className="text-sm text-foreground">
                            {applicant.cfr_comments || applicant.lr_comments || 'No comments'}
                          </span>
                        </td>
                        <td className="p-4">
