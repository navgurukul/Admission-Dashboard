...
const SchoolStages = () => {
  ...
  const closeEditDialog = () =>
    setEditModal({ open: false, idx: null, form: defaultEditStage });
  ...
  const closeTransitionDialog = () =>
    setTransitionModal({ open: false, idx: null });
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="md:ml-64 flex flex-col items-center justify-center min-h-screen pt-16 md:pt-0">
                      ...
                      >
                        ...
                        <td className="py-3 px-4 text-center">
                          <button
                            ...
                            onClick={() => openTransitionDialog(idx)}
                          >
                            Manage Transitions
                          </button>
                        </td>
            ...
            <form onSubmit={handleEditSubmit} className="w-full max-w-md">
                ...
                <div className="flex gap-6 mb-2">
                  <label className="flex items-center gap-2">
                    <input
                      ...
                      name="stageType"
                      value="Test"
                      checked={editModal.form.stageType === "Test"}
                      onChange={() => handleEditFormChange("stageType", "Test")}
                    />
                    Test
                  </label>
                  <label className="flex items-center gap-2">
                    <input
                      ...
                      name="stageType"
                      value="Interview"
                      checked={editModal.form.stageType === "Interview"}
                      onChange={() =>
                        handleEditFormChange("stageType", "Interview")
                      }
                    />
                    Interview
                  </label>
              ...
              <div className="mb-6">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleEditFormChange("stageName", e.target.value)
                  }
                  required
                />
              </div>
              <button
                type="submit"
                className="w-full bg-orange-500 text-white py-2 rounded font-semibold text-lg shadow mt-2"
              >
                ADD ADMISSION STAGE
              </button>
            </form>
        ...
        <Dialog
          ...
          onOpenChange={(v) => !v && closeTransitionDialog()}
        >
          <DialogContent>
            <form onSubmit={handleTransitionSave} className="w-full max-w-lg">
              ...
              <div className="flex flex-col md:flex-row gap-8">
                  ...
                  <div className="flex flex-col gap-2">
                    {stages.map((s, i) => (
                      <label key={s.id} className="flex items-center gap-2">
                        <input
                          type="checkbox"
                          checked={!!transitionChecks[i]}
                          onChange={() => handleTransitionCheck(i)}
                        />
                        {s.stageName}
                      </label>
                    ))}
                ...
                <div className="flex-1">
                  ...
                  <select
                    ...
                    onChange={handleTransitionPreview}
                  >
                    {stages.map((s, i) => (
                      <option key={s.id} value={s.stageName}>
                        {s.stageName}
                      </option>
                    ))}
                  </select>
              ...
              <button
                type="submit"
                className="mt-6 w-full bg-orange-500 text-white py-2 rounded font-semibold text-lg shadow"
              >
