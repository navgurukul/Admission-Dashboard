...
export const setupDefaultAdmin = async (adminEmail: string) => {
  try {
    ...
    const createRoleResponse = await fetch(`${API_BASE_URL}/roles/createRoles`, {
      method: 'POST',
      headers: {
    ...
    if (!registerUserResponse.ok) {
      throw new Error(`Failed to register admin user: ${registerUserResponse.status}`);
    }
...
export const initializeAdminSystem = async (adminEmail: string = "admin@example.com") => {
  try {
