...
const apiFetch = async (endpoint: string, options: RequestInit = {}, timeout = 8000) => {
...
export const difficultyLevelAPI = {
  ...
  async createDifficultyLevel(data: CreateDifficultyLevelData): Promise<DifficultyLevel> {
    try {
      const validationErrors = difficultyLevelUtils.validateDifficultyLevel(data);
  ...
  async updateDifficultyLevel(id: number, data: UpdateDifficultyLevelData): Promise<DifficultyLevel> {
    try {
      if (data.name) {
  ...
  async getActiveDifficultyLevels(): Promise<DifficultyLevel[]> {
    try {
      ...
      throw new Error(error.message || 'Failed to fetch active difficulty levels');
    }
  },

  // Toggle difficulty level status
  async toggleDifficultyLevelStatus(id: number, currentStatus: boolean): Promise<DifficultyLevel> {
...
export const difficultyLevelUtils = {
  ...
  getDifficultyColor(name: string): string {
    ...
    return colors[name.toLowerCase()] || 'bg-gray-100 text-gray-800 border-gray-200';
  },
