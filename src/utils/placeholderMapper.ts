...
export const getApplicantData = async (applicantId: string): Promise<ApplicantData | null> => {
  try {
...
export const replacePlaceholders = (htmlContent: string, applicantData: ApplicantData): string => {
  ...
  const placeholderMappings: Record<string, string | number | null> = {
    ...
    'INTERVIEW_DATE': applicantData.interview_date ? new Date(applicantData.interview_date).toLocaleDateString() : null,
